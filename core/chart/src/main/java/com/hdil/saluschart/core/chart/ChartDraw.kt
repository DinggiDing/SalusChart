...
object ChartDraw {
    ...
    fun drawPoints(drawScope: DrawScope, points: List<Offset>, values: List<Float>) {
        points.forEachIndexed { i, point ->
            ...
            val labelPosition = calculateTangentBasedLabelPosition(
                pointIndex = i,
                points = points,
                labelText = labelText
            )
     ...
     * @return 최적의 라벨 위치
     */
    private fun calculateTangentBasedLabelPosition(
        pointIndex: Int,
        points: List<Offset>,
        labelText: String
    ): Offset {
     ...
     * 벡터를 정규화합니다.
     */
    private fun normalizeVector(vector: Offset): Offset {
        val magnitude = kotlin.math.sqrt(vector.x * vector.x + vector.y * vector.y)
